 ...
 //    var storage = window.localStorage;  
    ...
    //-------------------
		...
		//图片地址切换
			...
			if($('.wz li').length>3){
					var ramNum=Math.floor(Math.random() *(arr[bb].gs));
					var tipstext=arr[bb].wz[ramNum]
					$('.wz li').eq(ramNum).css({'background':"url(./images/text-back_s.png) no-repeat",'background-size': '100% 100%'});
					$('.wz li').eq(ramNum).addClass('active')
					$('.wz li').eq(ramNum).html(tipstext);
					b++;
					for(var i=0;i<$('.wz1 li').length;i++){
						if($(".wz1 li").eq(i).html()==tipstext){
							$('wz1 li').eq(i).html('');
							$(".wz1 li").eq(i).css('visibility','hidden');
							break;
						}
					}
